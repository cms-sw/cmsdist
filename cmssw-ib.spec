...
DOW=`$PYTHON_ROOT/bin/python -c "import re; import os;from datetime import datetime ;print datetime.strptime(re.sub('.*([0-9]{4}-[0-9]{2}-[0-9]{2}-[0-9]{4}$)', '\\\\\1' , os.environ['CMSSW_VERSION']), '%Y-%m-%d-%H00').strftime('%a').lower()"`
HOUR=`$PYTHON_ROOT/bin/python -c "import re; import os;from datetime import datetime;print datetime.strptime(re.sub('.*([0-9]{4}-[0-9]{2}-[0-9]{2}-[0-9]{4}$)', '\\\\\1' , os.environ['CMSSW_VERSION']), '%Y-%m-%d-%H00').strftime('%H').lower()"`

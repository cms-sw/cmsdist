...
Requires: python py2-simplejson py2-sqlalchemy py2-httplib2 rotatelogs
pystack py2-sphinx dbs-client couchdb py2-pycurl couchskel py2-stomp
...
cd doc
cat asyncstageout/conf.py | sed "s,development,%{realversion},g" >
asyncstageout/conf.py.tmp
...
cp -pr ../AsyncStageout-%{realversion}/src/python/AsyncStageOut
...
#mkdir -p %i/bin
cp -pf %_builddir/WMCore-%{wmcver}/bin/{wmcoreD,wmcore-new-config,wmagent-mod-config,wmagent-couchapp-init}
%i/bin/
...
for tool in $(echo %{requiredtools} | sed -e's|\s+| |;s|^\s+||'); do
 ...
 if [ X"$r" != X ] && [ -r "$r/etc/profile.d/init.sh" ]; then
   echo "test X\$$root != X || . $r/etc/profile.d/init.sh" >>
%i/etc/profile.d/dependencies-setup.sh
   echo "test X\$$root != X || source $r/etc/profile.d/init.csh" >>
%i/etc/profile.d/dependencies-setup.csh
 fi
done

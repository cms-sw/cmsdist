...
cp -pr ../AsyncStageout/src/python/AsyncTransfer %i/$PYTHON_LIB_SITE_PACKAGES/
cp -pr ../AsyncStageout/src/couchapp %i/
find %i -name '*.egg-info' -exec rm {} \;

# Generate .pyc files.
python -m compileall %i/$PYTHON_LIB_SITE_PACKAGES/AsyncTransfer || true

mkdir -p %i/bin
cp -pf %_builddir/WMCore/bin/{wmcoreD,wmcore-db-init} %i/bin/
